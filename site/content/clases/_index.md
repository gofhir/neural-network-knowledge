...
{{< cards >}}
  ...
  {{< card link="clase-41" title="Clase 41 - Speech Recognition y Speaker Recognition" subtitle="Dos tareas opuestas sobre la misma señal: alinear audio con texto mediante el token blank y CTC, y colapsar un enunciado en un descriptor con VLAD. Cada una descarta lo que la otra necesita" icon="adjustments" >}}
  {{< card link="clase-42" title="Clase 42 - Tracking de Objetos en Video" subtitle="La identidad no se observa en ningún frame: hay que inferirla. Filtro de Kalman, algoritmo húngaro y descriptores de apariencia en SORT y DeepSORT — y la métrica que casi no nota la diferencia entre ambos" icon="eye" >}}
  {{< card link="clase-43" title="Clase 43 - Aplicaciones para Audio y Video" subtitle="Donde confluyen los dos hilos. Imagen y sonido describen la misma escena: SoundNet lo usa para entrenar sin etiquetas, E2E-AVSR para decidir cuando el audio se degrada" icon="sparkles" >}}
  {{< card link="clase-44" title="Clase 44 - Aplicaciones de Audio y Video (cierre)" subtitle="La última del diplomado. Siete aplicaciones donde la información no está en la entrada y la completa un prior: de la reconstrucción de voz a los deep fakes" icon="sparkles" >}}
{{< /cards >}}
