...
## Su continuación directa: TSM

La limitación estructural de TSN —el consenso por promedio es **invariante al orden**, de modo que reproducir el video al revés da idénticamente la misma salida— es el punto de partida de [TSM](/papers/tsm-lin-2019) (Lin, Gan y Han, ICCV 2019). TSM conserva **todo** el marco de TSN —el muestreo por segmentos, los parámetros compartidos, el consenso por promedio— y le inserta un [desplazamiento temporal](/fundamentos/desplazamiento-temporal) de 1/4 de los canales dentro de cada bloque residual, de modo que los *features* que producen cada predicción ya vienen contaminados por sus vecinos temporales. El costo es cero parámetros y cero FLOPs, y la ganancia sobre esta misma baseline va de +3,5 puntos en Kinetics a +28,0 en Something-Something.

La continuidad es literal en el código: la implementación oficial de TSM instancia una clase llamada **`TSN`** con el argumento `is_shift=True`. Ambos marcos se recorren en la [Clase 40](/clases/clase-40), y el [Laboratorio 40](/laboratorios/lab-40) mide la diferencia anulando el desplazamiento sobre un checkpoint entrenado, lo que convierte el modelo en un TSN 2D con los mismos pesos.
