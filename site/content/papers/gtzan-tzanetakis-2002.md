...
Vale un contraste con los números que efectivamente produjo el [laboratorio](/laboratorios/lab-37/03-gtzan-mfcc-vs-wav2vec): una GRU sobre MFCC alcanzó **19,31%** y sobre embeddings wav2vec 2.0 **28,62%** — bastante por debajo del ~61% del paper original con features hechas a mano y clasificadores estadísticos. La lectura correcta no es que el deep learning rinda peor, sino que el presupuesto no daba: el split "filtered" de torchaudio deja **443 clips de entrenamiento (44 por género)** y el lab entrena 10 épocas con `lr=1e-4`, o sea **230 pasos de gradiente** en total. La loss baja apenas 7,8% desde el valor de azar ($\ln 10 = 2{,}303$). Es un recordatorio útil de que un pipeline clásico bien ajustado puede superar a una red mal presupuestada, y de que las cifras de un paper traen implícito todo su régimen de entrenamiento.
